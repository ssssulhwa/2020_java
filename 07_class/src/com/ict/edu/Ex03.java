...
public class Ex03 {
	public static void main(String[] args) {
		// 이클립스에서 API  호출 : 해당 클래스에 shift + f2
		...
		// float와 double은 0.0 이상 1.0미만에서 난수 발생
		System.out.println("float형 : " + ran.nextFloat());
		System.out.println("double형 : " + ran.nextDouble());
		
		// 특정 범위를 지정하는 방법 : 
		// 1) nextInt(범위) : 0 ~ 범위 전까지 정수에서 난수 발생
		System.out.println("범위지정 :" + ran.nextInt(5)); // 0-4까지에서 난수 (5는 포함X)
		// 2) (int)(nextDouble() * 범위) : 0 ~ 범위 전까지 정수에서 난수 발생
		System.out.println("범위지정 :" + (int)(ran.nextDouble()*5));	// 0-4까지에서 난수 (5는 포함X)
		...
		System.out.println("반올림 " + Math.floor(3.55));	// 3
